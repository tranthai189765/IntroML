      ...
      cho mỗi WOEID (vùng: world/US/JP/BR/IN ...):
          ...
          cho mỗi topic (top 10):
              GET /twitter/tweet/advanced_search
                  query = "<topic> since_time:<now-30min>"  queryType=Top  (1 trang)
                  → chỉ post tuổi 0–30p, sắp theo engagement
...
python x_pipeline.py run --target 5000 --interval 1800 --budget 8.0
```

| Lệnh | Chức năng |
|---|---|
| `init` | Tạo CSDL + bảng (idempotent) |
| `intake` | Crawl post fresh 0–30p, lọc spam, tải ảnh, lưu + snapshot t0 |
...
nohup python -u x_pipeline.py run --target 20000 --interval 1800 --budget 40 \
      >> pipeline.log 2>&1 &
